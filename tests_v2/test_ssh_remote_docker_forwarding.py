...
def _ssh_remote_proxy_supported() -> bool:
    try:
    ...
    return bool(features.get("ssh_remote_proxy", features.get("ssh_remote_full", True)))
...
def main() -> int:
    if not _docker_available():
        ...
        return 0
    if not _ssh_remote_proxy_supported():
        print("SKIP: cmux build does not advertise SSH remote proxy support")
        return 0
