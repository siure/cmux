...
- [x] Evaluate feasibility of request interception/mocking in `WKWebView`; implement supported subset.
  - Supported subset: route/unroute by URL pattern, synthetic response bodies,
    request log inspection, HAR-style entries, and response-body reads.
  - Linux coverage: `browser_network_routes_and_request_log_are_observable`.
- [x] Add emulation settings that are feasible in `WKWebView`.
  ...
  - Linux live coverage: `webkit-runtime-smoke` verifies initial, immediate,
    ...
    fallback.
- [x] Add trace/recording equivalents where practical.
  - Supported subset: trace start/stop JSON artifacts, HAR artifacts,
    periodic native WebKit screencast/video frames with deterministic model
    fallback, and `browser record` start/stop/restart compatibility aliases.
    Frame payloads report `source: webkit|model`; recording generations reject
    late native callbacks after stop or restart.
