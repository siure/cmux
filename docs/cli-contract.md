...
| `browser press`, `browser key`, `browser keydown`, `browser keyup`, `browser keyboard` | Send keyboard input as `--key <key>` or positional `<key>` using Playwright/W3C names such as `Enter`, `Tab`, `Escape`, `ArrowLeft`, and `Space`. `Space`, `Spacebar`, and `space` emit DOM key `" "` with code `"Space"`; raw `--key ' '` is also accepted. |
...
| `hooks <agent> <event>` | Generic hook surface for `grok`, `opencode`, `pi`, `amp`, `cursor`, `gemini`, `kimi`, `rovodev`, `copilot`, `codebuddy`, `factory`, and `qoder`. `UserPromptSubmit` records a last-turn diff baseline when workspace/surface/cwd context is available; `PreToolUse` records one only if a baseline for the current turn is missing. |
