# Linux Feature and Command Reference

This is the comprehensive reference retained from the original Linux README.
Start with the short [README](README.md), use [Development](DEVELOPMENT.md) for
the contributor workflow, [Architecture](ARCHITECTURE.md) to find code, and the
[Roadmap](ROADMAP.md) for release scope.
...
The commands below are detailed reference material. Complete the toolchain and
native prerequisites in [Development](DEVELOPMENT.md#native-prerequisites)
first. Use `run-dev.sh` for source launches because it verifies the submodule
revision and provisions the exact Zig version. Before any standalone `zig`
command, development install, or bundle build, also follow the
[exact-Zig setup](DEVELOPMENT.md#development-install-and-bundle), because those
two scripts currently invoke `zig` from `PATH`.
...
For the complete native app, use the guarded development launcher from the
`cmux` checkout:
...
For a persistent user-local desktop install, first expose the exact Zig binary
as described in [Development](DEVELOPMENT.md#development-install-and-bundle):
...
```bash
cargo run --locked --manifest-path linux/Cargo.toml -- app
cargo run --locked --manifest-path linux/Cargo.toml --features gtk -- app --renderer gtk
CMUX_LINUX_RENDERER=ghostty-vt cargo run --locked --manifest-path linux/Cargo.toml -- app
```
...
```bash
cargo run --locked --manifest-path linux/Cargo.toml -- \
  feedback --email you@example.com --body "Linux feedback"
cargo run --locked --manifest-path linux/Cargo.toml -- feedback retry
```
...
```bash
cargo run --locked --manifest-path linux/Cargo.toml --features gtk -- \
  app --renderer ghostty --script $'renderer diagnostics\nquit'
```
...
Build the pinned Ghostty fork and cmux through the guarded launcher, then start
the GTK app on a private control socket:

```bash
CMUX_LINUX_SOCKET_PATH=/tmp/cmux-mvp.sock ./linux/scripts/run-dev.sh
```
...
The implementation has broad feature coverage, but the roadmap's audited
daily-driver contract is not yet proven. Full product parity also requires:
...
After completing the exact-Zig setup in
[Development](DEVELOPMENT.md#development-install-and-bundle), build a release
archive from the cmux tree and the local Ghostty checkout:
...
Earlier implementation work added checks for optimized cmux and `ReleaseSafe`
Ghostty prefixes, reproducible archives, relocation through paths containing
spaces, GTK/Ghostty launch, socket control, and arbitrary install prefixes. The
2026-08-20 ownership audit did not rerun that complete clean bundle and install
scenario, so the roadmap correctly keeps it open. Publication also requires the
pinned Ghostty commit to be pushed before the parent pointer, and `cmux update`
...
For a user-local Linux app launcher, first complete the exact-Zig setup in
[Development](DEVELOPMENT.md#development-install-and-bundle), then build and
install the source port under XDG paths:
...
`CMUX_LINUX_RENDERER=ghostty` (the default), it also builds the selected Ghostty
...
- A renderer-facing snapshot and diagnostics API (`renderer.snapshot` and
  ...
  produced by `zig build -Demit-lib-vt=true -Doptimize=ReleaseSafe` in the
  Ghostty checkout, verify the
...
- An optional GTK4 app shell (`cargo run --locked --manifest-path linux/Cargo.toml --features gtk -- app --renderer gtk`)
...
- Optional runtime-loaded `libghostty-vt` formatting through
  ...
  `zig build -Demit-lib-vt=true -Doptimize=ReleaseSafe` when present and
  preserving the Rust fallback
...
renderer-realized boolean alias. After the
[exact-Zig setup](DEVELOPMENT.md#development-install-and-bundle), build it with
`zig build -Dapp-runtime=none -Doptimize=ReleaseSafe` to produce
`ghostty-internal` for cmux diagnostics. cmux verifies the required full-embedding symbols from
...
for `cargo run --locked --manifest-path linux/Cargo.toml --features gtk -- app --renderer ghostty` that forwards keyboard,
